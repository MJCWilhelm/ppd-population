...
if __name__ == '__main__':
    ...
    new_kroupa_mass_distribution = lambda N: MultiplePartIMF(
        mass_boundaries=[0.08, 0.5, 100.] | units.MSun, alphas=[-1.3, -2.3], 
        random=True).next_mass(N)
    ...
    masses[ np.argmax(masses.value_in(units.MSun)) ] = 30. | units.MSun
    ...
    channels = {
        'ppd_to_star': ppd_code.star_particles.new_channel_to(stars),
        'star_to_ppd': stars.new_channel_to(ppd_code.star_particles),
        'grav_to_star': gravity_code.particles.new_channel_to(stars),
        'star_to_grav': stars.new_channel_to(gravity_code.particles)
    }
    ...
    n = int(t_end/dt)
    disk_radii = np.zeros((n, len(ppd_code.disked_stars)))
    disk_masses = np.zeros((n, len(ppd_code.disked_stars)))
    epe_rate = np.zeros((n, len(ppd_code.disked_stars)))
    grav_mass = np.zeros((n, N))
    ...
    while time < t_end:
        ...
        gravity_code.evolve_model( time + dt/2. )
        channels['grav_to_star'].copy()
        ...
        disks = ppd_code.disk_particles

        disk_radii[counter] = disks.disk_radius.value_in(units.AU)
        disk_masses[counter] = disks.disk_gas_mass.value_in(units.MSun)
        epe_rate[counter] = disks.outer_photoevap_rate.value_in(units.MSun/units.yr)
        grav_mass[counter] = stars.mass.value_in(units.MSun)
    ...
    plt.plot(disk_radii, disk_masses)
    plt.yscale('log')
    plt.xscale('log')
